...
## Hero bündig zum Viewport – 14.09.2026

- Begrenzte Desktop-Mindesthöhe und inhaltsabhängige Mobilhöhe durch die aktuelle Viewporthöhe abzüglich gemessener Headerhöhe und 1px Abschlusslinie ersetzt. Dynamische Browserhöhe wird mit dvh berücksichtigt; svh/vh dienen als Fallback. Header wird sofort und anschließend per ResizeObserver gemessen.
- Deutlich größere, von Breite und Höhe abhängige Typografie. Desktop-Portrait nach verfügbarer Höhe skaliert, maximal 480px breit. Mobil und im Tablet-Hochformat bilden Titel und Film eine überlagerte Komposition mit leseförderndem Verlauf. Pauseknopf bleibt frei oben rechts; Caption für Screenreader erhalten. CTA-Bereich sitzt unten.
- 78 Layoutkontrollen in Chromium, Firefox und WebKit bestanden: je 23 Formate zwischen 320 und 2560px, darunter niedrige Desktopfenster, kurze Handy-Querformate und sehr hohe Mobilfenster; zusätzlich je drei Breiten mit 200% CSS-Basisschrift. Auf normaler Schriftgröße schließt der Hero in sämtlichen geprüften Formaten innerhalb 1px bündig mit der Fensterunterkante ab. Keine festgestellten horizontalen Überläufe oder abgeschnittenen kontrollierten Texte/Links.
- Nach Neuladen bei 393 × 852 und 1440 × 900 sowie beim Wechsel zwischen Fensterhöhen und Ausrichtungen erneut geprüft. Autostart, Pause/Fortsetzen, Menü, Escape und Ankersprung zum Film unter dem Header funktionieren in allen drei Engines. Keine JavaScript-Laufzeitfehler.
...
## Robuster Autoplay-Start – 14.09.2026

- Native Autoplay-Attribute im Hero ergänzt; muted/defaultMuted/playsInline vor jeder Wiedergabe synchronisiert. Der Controller startet anhand der tatsächlichen Geometrie sofort und übernimmt einen bereits laufenden Clip ohne falschen Ladezustand. Ohne IntersectionObserver greift eine Sichtbarkeitssteuerung über Scroll-/Resize-Ereignisse.
- 21 gezielte Browserprüfungen bestanden, je sieben in Chromium, Firefox und WebKit: verzögerter Controller nach nativem Start, fehlender IntersectionObserver, browserseitige Pause mit anschließendem Ein-Klick-Neustart, erhaltene manuelle Pause und simulierte NotAllowedError-Sperren mit anschließender echter Klick-, Touch- oder Tastatureingabe. Keine endlose Wiederholung abgewiesener Starts.
- 18 zusätzliche Wiedergabeprüfungen: Start-, Studio- und Filmproduktionsseite bei 393 × 852 und 1440 × 900 in allen drei Engines. Stumme Inline-Wiedergabe, Pause/Fortsetzen und simulierte pagehide/pageshow-Rückkehr bestanden. Manuelle Pause bleibt auch nach diesen Ereignissen bestehen. Die Ereignissimulation ist kein Nachweis eines echten BFCache-Eintrags.
- Native Wiedergabe ohne JavaScript in allen drei Engines beobachtet. Legacy-play() ohne Promise, synchron geworfene Ablehnung und wiederholte AbortError-Unterbrechungen in Chromium geprüft: kein Laufzeitfehler und keine unendliche Retry-Schleife. Ein zunächst mit requestAnimationFrame gepollter No-JS-Test wurde auf direkte Zustandsabfrage umgestellt, da dieser Pollmodus bei deaktiviertem JavaScript nicht zuverlässig lief.
- Langsames Laden sowie ein Videofehler vor Ausführung des Medien-Skripts erneut simuliert. Standbild, tatsächlicher Ladezustand, SVG-Regler und erneutes Laden per Klick funktionieren. Desktop- und mobile WebKit-Ansicht visuell kontrolliert; Gestaltung und Hero-Höhe bleiben unverändert.
- Sichtbarkeitsgrenzen in Chromium geprüft: ein sichtbarer Playbutton startet das Studiovideo auch bei nur14,86% sichtbarer Videofläche; vollständig außerhalb des Fensters pausiert es wieder. Echter CDP-Zoom auf2× mit Touch-Panning bestätigt die korrigierte Visual-Viewport-Rechnung:76,37% sichtbare Fläche statt fälschlich0%, Wiedergabe läuft weiter. Keine Laufzeitfehler.
