...
const {origin}=JSON.parse(readFileSync(fileURLToPath(new URL('../site.config.json',import.meta.url)),'utf8'));
...
if(!readFileSync(join(root,'robots.txt'),'utf8').includes(`Sitemap: ${origin}/sitemap.xml`))errors.push('Robots sitemap declaration missing');
